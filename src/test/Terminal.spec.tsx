...
describe("Terminal Component", () => {
  ...
  describe("Input Commands", () => {
    ...
    it("should return '/home/bes' when user type 'pwd' cmd", async () => {
  ...
  describe("Redirect commands", () => {
    ...
    const numsProjects = [1, 2, 3];
    const numsSocials = [1, 2];
    numsProjects.forEach(numsProjects => {
      it(`should redirect to project URL when user type 'projects go ${numsProjects}' cmd`, async () => {
        await user.type(terminalInput, `projects go ${numsProjects}{enter}`);
        expect(window.open).toHaveBeenCalled();
      });
    });

    numsSocials.forEach(numsSocials => {
      it(`should redirect to social media when user type 'socials go ${numsSocials}' cmd`, async () => {
        await user.type(terminalInput, `socials go ${numsSocials}{enter}`);
        expect(window.open).toHaveBeenCalled();
      });
    });
  });
  ...
  describe("Invalid Arguments", () => {
    ...
    specialUsageCmds.forEach(cmd => {
      ...
      it(`should return usage component for '${cmd}' cmd with incorrect option`, async () => {
        const arg = cmd === "themes" ? "go light" : "set 2";
        window.open = vi.fn();
